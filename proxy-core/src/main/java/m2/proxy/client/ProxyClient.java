...
public class ProxyClient extends TcpBaseClientBase {
    ...
    public ProxyClient(
            String clientId, String serverAddr, int tcpPort, DirectForward directForward, LocalSite localSite
                      ) {
            ...
            protected void onRequest(long sessionId, long requestId, RequestType type, String address, ByteString requestBytes) {
                getServer().getTaskPool().execute( () -> {
                    try {
                        ...
                        if (!ret.isPresent()) {
                        ...
                        } Thread.sleep( TcpBase.rnd.nextInt( 10 ) + 1 ); if (!ret.isPresent()) {
                            ...
                            ByteString reply = ByteString.copyFromUtf8(
                                    directForward.makeErrorReply( "not found" + request.getUri() ).toString() );
                            reply( sessionId, requestId, type, reply );
                        }
                    } catch (HttpException | InterruptedException e) {
                        log.warn( "Error request: {}", e.getMessage() ); ByteString reply = ByteString.copyFromUtf8(
                                directForward.makeErrorReply( e.getMessage() ).toString() );
                        reply( sessionId, requestId, type, reply );
                    }
                } );
            }
